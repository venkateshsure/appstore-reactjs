...
class AppStore extends Component {
  state = {
    initialTabId: appsList[0].category,
    input: '',
  }

  getAppsList = () => {
    const {initialTabId} = this.state
    const getList = appsList.filter(each => each.category === initialTabId)
    return getList
  }
  ...
  render() {
    const {initialTabId, input} = this.state
    const filteredAppsList = this.getAppsList()
    const searchResults = filteredAppsList.filter(each =>
      each.appName.toLowerCase().includes(input.toLowerCase()),
    )

    return (
      <div className="app-store">
        <div className="app-store-con">
          ...
          <ul className="ul-con">
            {tabsList.map(each => (
              <TabItem
                ...
                key={each.tabId}
                each={each}
                isActive={each.tabId === initialTabId}
              />
            ))}
          </ul>
          <ul className="ul-con-app">
